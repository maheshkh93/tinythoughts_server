...
export async function getPagedItemsFromCollection(
  collectionName,
  pageNumber,
  query = {}
) {
  ...
  return new Promise((resolve, reject) => {
    MongoClient.connect(process.env.DB_CONNECTION_STRING, function (err, db) {
      ...
      dbo
        ...
        .deleteOne(matchQuery, function (delete_err, obj) {
          if (delete_err) {
            reject(delete_err);
            throw delete_err;
          }
...
export async function updateItemFromCollection(
  collection,
  matchQuery,
  partial
) {
  return new Promise((resolve, reject) => {
    MongoClient.connect(process.env.DB_CONNECTION_STRING, function (err, db) {
      ...
      dbo
        .collection(collection)
        .updateOne(matchQuery, { $set: partial })
        .then((res) => {
          resolve(res);
        })
        .catch((er) => reject(er));
